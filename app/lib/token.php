...
class token extends base
{
     ...
     * @param int $life
     */
    public function __construct(int $life = 0)
    {
        parent::__construct();
        //todo 需要修改 kengen， 不要用开源的
    ...
    public function make(array $data, string $key): string
    {
        ...
        //延长token周期
        $this->redis->set($token_key, hash('md5', $token), $this->life);
    ...
    public function parse(string $token, string $key): array
    {
        ...
        //延长token周期
        $this->redis->expire($token_data['key'], $this->life);
